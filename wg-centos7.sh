...
new_client_setup() {
        ...
        if [ "$1" = "add_client" ]; then
                ...
                if [[ ! "$specify_ip" =~ ^[yY]$ ]]; then
                        echo "Using auto assigned IP address 10.8.0.$octet."
                fi
        fi
        if [[ "$specify_ip" =~ ^[yY]$ ]]; then
                echo
                read -rp "Enter IP address for the new client (e.g. 10.8.0.X): " client_ip
                ...
                until [[ $client_ip =~ ^10\.7\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]] \
                        ...
                        if [[ ! $client_ip =~ ^10\.7\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]]; then
                                echo "Invalid IP address. Must be within the range 10.8.0.2 to 10.8.0.254."
                        else
...
PresharedKey = $psk
AllowedIPs = 10.8.0.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = 10.8.0.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key
...
[Interface]
Address = 10.8.0.1/24$([[ -n "$ip6" ]] && echo ", fddd:2c4:2c4:2c4::1/64")
